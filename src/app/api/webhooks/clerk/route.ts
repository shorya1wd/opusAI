...
export async function POST(req: Request) {
  // We will get this secret from the Clerk Dashboard in Step 3
  ...
  // Get the Svix security headers
  const headerPayload = await headers()
  const svix_id = headerPayload.get("svix-id")
  const svix_timestamp = headerPayload.get("svix-timestamp")
  const svix_signature = headerPayload.get("svix-signature")

  // If there are no headers, immediately reject the request
  ...
  // Initialize Svix with our secret
  ...
  if (eventType === 'user.created') {
    ...
    try {
      // 🛡️ Upsert ensures we never throw a unique constraint error if a user exists
